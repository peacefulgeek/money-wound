/**
 * bulk-seed.mjs
 * Generates 500 articles using DeepSeek V4-Pro, passes them through the Paul Voice Gate,
 * and inserts them as status='queued' for the cron to trickle out.
 *
 * Run: node scripts/bulk-seed.mjs
 * Or as a DigitalOcean/Railway one-off: npm run bulk-seed
 *
 * IMPORTANT: This script is designed to run once to pre-fill the queue.
 * The cron will publish one article at a time from the queue.
 */
import { query } from '../src/lib/db.mjs';
import { generateArticleBody, generateMetaDescription } from '../src/lib/deepseek-generate.mjs';
import { cleanAndGate } from '../src/lib/article-quality-gate.mjs';

// ─── ASIN Pool ────────────────────────────────────────────────────────────────
const ASIN_POOL = [
  '0143115766', '1523085819', '0062935291', '1401952461', '0767920104',
  '1250077060', '0385342535', '1250301939', '0316159212', '1629144460',
  '0767921410', '0062409603',
];
...
const TOPICS = [
  ...
  "The Relationship Between Compassion and Financial Healing",
];
...
async function run() {
  ...
  console.log(`[bulk-seed] Model: ${process.env.OPENAI_MODEL || 'deepseek-v4-pro'}`);
  console.log(`[bulk-seed] All articles will be inserted as status='queued'`);
  console.log('');
  ...
  for (let i = 0; i < TOPICS.length; i++) {
    ...
    if (existing.length > 0) {
      console.log(`${progress} SKIP (exists): ${topic}`);
      queued++;
      continue;
    }
    ...
    while (attempt < MAX_ATTEMPTS && !success) {
      ...
      try {
        ...
        if (!pass) {
          console.warn(`${progress} Gate FAIL (attempt ${attempt}): ${failures.slice(0, 2).join(', ')}`);
          continue;
        }
        ...
        console.log(`${progress} QUEUED: ${slug} (${wordCount}w, ${elapsed}s elapsed)`);
        queued++;
        success = true;
      } catch (err) {
  ...
  console.log('');
  console.log(`[bulk-seed] Complete: ${queued} queued, ${failed} failed (${totalTime}s total)`);
